...
function Head({ pageTitle }: HeadTypes) {
    ...
    const [title, setTitle] = useGlobalState<string>("title", localStorage.getItem("metallic/title") || "Google Classroom");
    const [icon, setIcon] = useGlobalState<string>("icon", localStorage.getItem("metallic/icon") || "/assets/cloakingIcons/googleclassroom.png");
    ...
    useEffect(() => {
        if (title) {
            ...
            if (pageTitle) {
                window.document.title = "Very Difficult Math Problems";
            } else {
                window.document.title = "Very Difficult Math Problems";
            }
        }
